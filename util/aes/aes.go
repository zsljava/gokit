...
func processKey(key string) []byte {
	...
	case 16, 24, 32:
		return k // 已经是有效长度
	default:
		// 在16-32之间但不是标准长度，补充到下一个标准长度
		if len(k) <= 16 {
			return append(k, bytes.Repeat([]byte{0}, 16-len(k))...)
		} else if len(k) <= 24 {
			return append(k, bytes.Repeat([]byte{0}, 24-len(k))...)
		} else if len(k) <= 32 {
			return append(k, bytes.Repeat([]byte{0}, 32-len(k))...)
		} else {
			// 超过32位，截取前32位
			return k[:32]
		}
